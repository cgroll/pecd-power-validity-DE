...
Adapted from ~/research/pecd-replication/pecdr/cds.py: that project's
connection to cds.climate.copernicus.eu is intermittently flaky at the TLS
layer, not something a request payload change fixes, so every CDS call
wraps `client.retrieve()` with linear backoff instead of retrying manually.
"""
